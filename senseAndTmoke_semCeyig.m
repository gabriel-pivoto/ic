%% =====================================================================
% MATLAB + COMSOL - 4D Hierarchical Search (TMOKE + angular sensitivity)
% ---------------------------------------------------------------------
% Goal (one line):
%   Find the 4D geometry that simultaneously maximizes (A) |TMOKE| and
...
figureOutputDirectory     = fullfile(projectRootDir,'plots', ['tmoke_sens_4d_sem_ceyig_' runTimestamp]);
...
if SAVE_SNAPSHOT
    ...
    snapshotFilePath = fullfile(projectRootDir, sprintf( ...
        'snapshot_4d_sem_ceyig_bestTradeoff_Ldom%4.0f_Lden%4.0f_hsi%3.0f_hau%.3f_n%.3f_alpha%.4f_%s.mph', ...
...
if ~isempty(coarseResultsTable), writetable(coarseResultsTable, fullfile(projectRootDir,'tmoke_sens_4D_sem_ceyig_coarse.csv')); end
if ~isempty(fineResultsTable),   writetable(fineResultsTable,   fullfile(projectRootDir,'tmoke_sens_4D_sem_ceyig_fine.csv')); end
if ~isempty(superResultsTable),  writetable(superResultsTable,  fullfile(projectRootDir,'tmoke_sens_4D_sem_ceyig_super.csv')); end

writetable(bestDenseTable, fullfile(projectRootDir,'tmoke_sens_4D_sem_ceyig_bestTradeoff_dense_ALLn.csv'));
writetable(bestFullTable,  fullfile(projectRootDir,'tmoke_sens_4D_sem_ceyig_bestTradeoff_full_baseline.csv'));
...
% ==================================================================
if SAVE_FIGS
    saveAllOpenFigures(figureOutputDirectory, "tmoke_sens_4d_sem_ceyig", FIG_FORMATS);
...
function saveStageCandidatePlots(T, stageName, outDir, formats)
    ...
    fig = figure('Name', sprintf('%s candidate metrics', stageName), ...
    ...
    yyaxis left;
    plot(candidateIndex, abs(T.maxAbsTMOKE_base), '-', 'LineWidth', 1.1);
    ylabel('|TMOKE|_{max}');
    yyaxis right;
    plot(candidateIndex, abs(T.S_est_deg_per_RIU), '-', 'LineWidth', 1.1);
    ylabel('|S_{est}| [deg/RIU]');
    xlabel('Candidate index');
    title(sprintf('%s metrics by candidate', stageName));
    grid on;

    nexttile;
    scatter(abs(T.maxAbsTMOKE_base), abs(T.S_est_deg_per_RIU), 28, 'filled');
    xlabel('|TMOKE|_{max}');
    ylabel('|S_{est}| [deg/RIU]');
    title(sprintf('%s trade-off map', stageName));
    grid on;

    saveFigureAndClose(fig, outDir, sprintf('%s_candidate_metrics', lower(stageName)), formats);
end
